...
void main(void) {
    if (vTextureCoor.x > vTextureCoor.y) {
        gl_FragColor = texture2D(colorMap0, vTextureCoor);
    } else {
        gl_FragColor = texture2D(colorMap1, vTextureCoor);
    }
    
//    gl_FragColor = texture2D(colorMap1, vTextureCoor) + texture2D(colorMap0, vTextureCoor);
