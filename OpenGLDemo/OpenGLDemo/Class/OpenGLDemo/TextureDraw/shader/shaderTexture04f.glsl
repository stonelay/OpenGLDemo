...
//    for (int i = -1; i <= 1 ; i++) {
//        for (int j = -1; j <= 1; j++) {
//            float x = vTextureCoor.x + float(i) * delta;
//            float y = vTextureCoor.y + float(i) * delta;
...
//    for (int i = -1; i <= 1; i++) {
//        for (int j = -1; j <= 1; j++) {
//            float x = max(0.0, vTextureCoor.x + float(i) * delta);
//            float y = max(0.0, vTextureCoor.y + float(i) * delta);
//            color += texture2D(colorMap0, vec2(x, y)) * factor[(i+1)*3+(j+1)];
//        }
//    }
//
//    //gl_FragColor = texture2D(image, vTextureCoor);
